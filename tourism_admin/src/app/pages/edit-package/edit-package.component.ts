...
export class EditPackageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.obj.packageSingle(this.pkid).subscribe((res)=>{
      ...
      this.packageForm.controls.name.setValue(this.package.at(0).name);
      this.packageForm.controls.type.setValue(this.package.at(0).type);
      this.packageForm.controls.location.setValue(this.package.at(0).location);
      this.packageForm.controls.price.setValue(this.package.at(0).price);
      this.packageForm.controls.feature.setValue(this.package.at(0).feature);
      this.packageForm.controls.descp.setValue(this.package.at(0).descp);
      this.imageSrc="http://localhost:5000/public/images/package/"+this.package.at(0).image;
    });
  }
  ...
  clear() {
    ...
    this.packageForm.controls.image.addValidators([Validators.required]);
    this.imageSrc = '';
    this.image = '';
    if(this.image != null)
    {
  ...
  saveData() {
    ...
    if (this.packageForm.valid) {
      ...
      this.obj.packageEdit(this.pkid,data).subscribe((data) => {
        if (data["status"] == true) {
          this.status = true;
        }
      });
